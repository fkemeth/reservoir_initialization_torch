...
expected_dim = 3 - 2 * (parameters["sigma"] + parameters["beta"] + 1) / (
    parameters["sigma"] + 1 + np.sqrt((parameters["sigma"] - 1) ** 2 + 4 * parameters["sigma"] * parameters["rho"])
)
...
def max_lyap(pars, ic, stepper, d0=1e-8, nstep=1e4, dTarr=np.linspace(0.04, 0.16, 4), plot_flag=False):
...
nolds.lyap_r(
    trajectory, tau=time_array[1] - time_array[0], debug_plot=True, min_tsep=1000, emb_dim=5, lag=1, trajectory_len=4
)
...
trajectory = lorenz.integrate(
    create_initial_condition(), np.linspace(0, 0.01 * len_trajectories, len_trajectories + 1), parameters
)[:, 0]
...
trajectory = lorenz.integrate(
    create_initial_condition(), np.linspace(0, dt * len_trajectories, len_trajectories + 1), parameters
)[:, 0]
nolds.lyap_r(trajectory, min_tsep=1000, emb_dim=5, tau=dt, lag=5, debug_plot=True, trajectory_len=40)
nolds.lyap_r(trajectory, emb_dim=5, tau=dt, debug_plot=True, trajectory_len=30, fit="poly", fit_offset=10)
